(ns ferry.operation
  "OperationActor -- one ferry-operations-coordination operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (FerryOperationsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Maritime Safety
  ...
  One graph run = one ferry-operations op (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A sailing's life is advanced
  by MANY ops (log / schedule / flag-concern / coordinate-maintenance),
  ...
  decision to a human operator (ferry operations dispatcher). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:schedule-sailing-operation`, `:flag-maritime-safety-
  concern` and `:coordinate-maintenance` ALWAYS reach this node when the
  governor is clean -- see `ferry.phase` (only `:log-voyage-record` may
  ever auto-commit).

  NOTE the effect/op dispatch split, deliberate for this actor: every
  proposal's `:effect` is ALWAYS the literal keyword `:propose` (a HARD
  governor invariant -- see `ferry.governor`), so `commit-record`
  includes the REQUEST's `:op` alongside it; `ferry.store/commit-
  record!` dispatches its SSoT mutation on that `:op`, not on `:effect`
  (which carries no discriminating information here by design)."
...
(defn build
  ...
  `ferry.store/Store`).
  opts:
    :advisor      -- a `ferry.ferryadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (ferryadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FerryOperationsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (ferryadvisor/-advise advisor store request)]
            {:proposal p :audit [(ferryadvisor/trace request p)]})))

      ;; Maritime Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
